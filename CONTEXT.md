...
- 2026-09-06 — **Modo horizontal (landscape)**: `screenOrientation="sensor"` + `configChanges` (não recria a Activity ao girar, preservando a transmissão WebRTC). Criado `res/layout-land/activity_main.xml` com mesmos IDs e menu compacto (status+botão numa linha, ícones de câmera em linha, AVANÇADO/Dispositivos lado a lado). Em `onConfigurationChanged` o app re-infla o layout preservando status, sessão, preview (sink re-adicionado), painéis e listas. A câmera NÃO gira — o giro de 90° e espelhamento da transmissão ficam por conta dos botões (envio ao receiver).